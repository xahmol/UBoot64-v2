...
| `uii_add_partition(index, name, path)` | SOFTIEC | Add (or idempotently re-point) a SoftIEC partition (firmware 3.15+); see §9's `filebrowse.c` section for how the browser uses this. No `uii_del_partition()` wrapper is provided even though `SOFTIEC_CMD_DEL_PARTITION` exists in the protocol -- confirmed on real hardware, even from a clean power-cycle, that it does not actually remove the partition from the live table; see the "not persistent" note below for why that's fine to live without |
...
| `char *pathconcat()` | Concatenate `path[]` array to a single path string; uses `uii_get_path()` in UCI mode. For SoftIEC/VICE devices, builds `cd://level1/level2/...` -- double slash, not single: firmware's `iec_path_to_fs_path()` (`software/io/iec/iec_channel.cc` in github.com/GideonZ/1541ultimate) strips a single leading `/` and treats the rest as relative to the drive's *current* directory, not its root; only `//` resolves to true root. A single slash worked for years only because the drive's current directory happened to already be at root the first time a slot ran; it silently breaks once something (e.g. `iec_select_partition()`, firmware 3.15+) has moved the current directory away from root before this command runs (fixed v3.0.1, discovered via the SoftIEC root partition boot-time fix above) |
...
**`CH_F3`'s auto-provisioned partition is triggered by, not on, the F8 toggle:** `cfg.iec_root_partition` is only a saved preference; the actual `uii_add_partition()` call happens the next time `CH_F3` transitions into IEC mode on a `U64` device with `!currentpartition`, not the moment the config screen's F8 key is pressed. This caused real user confusion during testing ("adding doesn't work anymore") when the toggle was flipped without a subsequent fresh F3 transition — worth remembering if this surfaces again.

**The auto-provisioned partition is not persistent.** `uii_add_partition()`/`SOFTIEC_CMD_ADD_PARTITION` only changes the Ultimate's live, in-memory partition table (`IecFileSystem`'s `partitions[]` array in firmware). Persisting to flash (`/Flash/Config/iec_partitions.ipr`, a small JSON file: `{"version":1,"partitions":[{"number":N,"path":"...","name":"..."}]}`) only happens via `IecFileSystem::SavePartitions()`, called exclusively from `MENU_IEC_SAVE_PAR` — a menu action gated on `cmd->user_interface`, i.e. only reachable from the Ultimate's own on-screen UI. There is no UCI command that triggers a save; confirmed on hardware (partition gone after a power cycle unless manually saved), and confirmed as intentional firmware behavior directly by Gideon Zweijtzer (firmware author, 2026-09-07): applies equally to partitions created via UCI or via the Ultimate's own menu UI, by design, since different applications may want different partition sets (hence `SavePartitions()` taking a filename). A future firmware version may add an automatic `.ipr` load per program (mirroring an existing automatic `.cfg`-load-per-program mechanism, unrelated to `CTRL_CMD_LOAD_CONFIG`) — worth revisiting if that ships, since it would let UBoot64 ship its own persisted partition file without ever touching the user's main partition table; not implemented as of this writing since the feature doesn't exist yet. Deliberately not worked around by having UBoot64 read/write this file directly: doing so safely would require a full read-parse-merge-rewrite (to avoid clobbering the user's own saved partitions, like a real second entry found on the test hardware) and a hand-rolled JSON parser (Oscar64 has none) for a format this project doesn't control the evolution of — the failure mode of a parser bug (corrupting the user's real saved partition table) is worse than the current one (an in-memory partition that just needs recreating, harmlessly, every boot). Instead, `CH_F3`'s conflict-check already treats "partition 254 already exists as UBoot64's own" as a match, not a conflict, and `uii_add_partition()` is idempotent — so if the user saves the partition table once via the Ultimate's own UI after UBoot64 creates it, every future boot's check simply recognizes it's already correct and does nothing further, achieving persistence with no additional code. **Firmware 3.15a changed what identifies "already ours" here**: `PR #878` switched the `"$=P"` listing's quoted name field from the partition's root path (`prt->GetRootPath()`, what 3.15 sends) to its configured Name (`prt->GetName()`, what 3.15a+ sends). Since `uii_add_partition(RESERVED_ROOT_PARTITION, UBOOT_PARTITION_NAME, "/")` sets Name `"UBOOT"` and path `"/"`, the conflict-check accepts either string as non-conflicting, so it recognizes UBoot64's own partition correctly on both firmware behaviors without needing to detect the firmware version. Before this fix (present through v3.0.0), 3.15a's listing always reported `"UBOOT"` where the check expected `"/"`, so auto-provisioning always misfired as "already in use by something else" on 3.15a+ — fixed in v3.0.1. An on-screen reminder about this was tried directly after the `uii_add_partition()` call and then removed (2026-09-07): it broke the layout (the side-panel-clearing rect it reused was sized for the main listing pane, not a full-screen message, so its text ran into the side panel) and, on reflection, this behavior belongs in the manual rather than as a runtime interruption users can't preview before turning F8 on — see README.md's F8 section instead.

**`UBOOT_PARTITION_NAME` (`src/core.c`) is a single shared constant, not a `"UBOOT"` literal re-typed at each call site.** Every occurrence of a plain string literal is an independent compile-time transformation under whichever charmap is active at that point in that file; this project's default petscii.h charmap (installed everywhere, case-inverting) gives no guarantee that two textually-identical `"UBOOT"` literals in different files -- or even different functions of the same file -- fold to the same bytes as each other, let alone as `parts[idx].path` (a raw wire string, never charmap-adjusted). This was a real, confirmed-on-hardware bug during 3.15a testing (2026-09-13): the conflict check intermittently misfired against UBoot64's own, correctly-created partition. Fixed by defining `UBOOT_PARTITION_NAME` once, under an identity-charmap override, in `core.c` (bank 0, so both `filebrowse.c` and `slotmenu.c` reference the identical bytes) rather than at each of the three sites (`filebrowse.c`'s create call, `filebrowse.c`'s compare, `slotmenu.c`'s boot-time create) independently.
