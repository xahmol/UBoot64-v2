...
void dir_waitfordriveready()
{
  cwin_putat_string(&cw, 0, 2, "Waiting for drive...", cfg.colors.text);
  delay(2);
...
//        device = device number
{
  ...
  // cwin_console_printf(&cw, cfg.colors.text, "Dev %d LFN %d ", device, lfn);
  ...
  // cwin_console_printf(&cw, cfg.colors.text, "St %d Er %d ", status, error);
...
// Output: 0 on success, 1 on failure
{
  ...
  memset(l_dirent->name, 0, MAXFILENAME);
  while (1)
  {
    // read byte
    b = krnio_chrin();
...
// Output: 1 on success, 0 on failure
{
  ...
  if (!fb_uci_mode)
  {
    memset(presentdir.name, 0, sizeof(presentdir.name));
    ret = dir_open(2, device);
  ...
  while (1)
  {
    ...
    if (!presentdir.name[0] && !fb_uci_mode)
    {
      // initialize directory in IEC mode
      if (presentdirelement.meta.type == CBM_T_HEADER)
      {
        sprintf(presentdir.name, "%.15s,%s", presentdirelement.name, disk_id_buf);
        presentdir.name[21] = 0;
        continue;
      }
      else
      {
        strcpy(presentdir.name, "Unknown type");
        break;
      }
    }
...
// Draw the frame for the directory listing
{
  ...
  {
    cwin_console_printf(&cw, cfg.colors.text, "[%02i]%.20s", device, presentdir.name);
...
// Go to bottom of dir in active pane
{
  ...
  if (presentdir.firstelement && presentdirelement.meta.next)
  {
    ...
    do
    {
      ...
      {
        count = 1; // Final entry found
      }
      cwin_cursor_move(&cw, 0, 2);
      cwin_console_printf(&cw, cfg.colors.text, "Pres: %lu Next: %lu  ", present, presentdirelement.meta.next);
      cwin_putat_string(&cw, 0, 2, " ", cfg.colors.text);
    } while (!count);
